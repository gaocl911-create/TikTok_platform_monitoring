...
阶段四已经能够采集抖音公开主页的真实账号指标，但真实作品列表、任务去重、失败重试和运行可观测性仍需完善。
本阶段负责补齐真实内容数据基础，为阶段五的评论监控与运营分析提供可靠输入。

### 工作内容

1. 实现抖音真实公开作品列表解析。
2. 保存作品 ID、标题、链接、发布时间、封面和公开互动指标。
3. 识别新作品、作品指标变化和疑似删除或下架作品。
4. 真实内容不可用时明确记录原因，禁止使用模拟作品补全。
5. 为 Celery 采集任务增加同账号并发去重。
6. 为临时失败增加有限次数重试和指数退避。
7. 对超时、页面结构变化、账号不匹配和需要人工验证等失败进行分类。
8. 连续采集失败达到阈值时生成系统预警。
9. 增加采集运行记录 API 和前端运行记录页面。
10. 展示任务状态、执行耗时、数据来源、结果摘要与失败原因。
...
- 不保存平台登录态或用户凭证。
- 平台公开作品不可用时，系统标记为部分可用或失败，不写入假数据。

### 验收标准

- 至少一个真实抖音账号可以发现并保存公开作品。
- 重复采集不会重复创建同一作品。
- 同一账号不会被多个 Celery 任务同时采集。
- 临时失败能够自动重试，永久失败不会无限重试。
- 连续失败达到阈值时能够生成系统预警。
- 可以在前端查看最近采集任务、执行耗时与失败原因。
- 真实作品不可用时不会生成模拟内容。

实施记录见 [docs/PHASE_4_5.md](./docs/PHASE_4_5.md)。
